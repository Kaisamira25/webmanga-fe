...
function AdminGift() {
    ...
    const isCoverExist = () => {
        return Gifts.some(item => item.GiftName.toLowerCase() === promotionalGiftName.toLowerCase());
    }
    const handleAddGift = async () => {
        console.log(promotionalGiftName, promotionalGiftType)
        if (promotionalGiftType.trim() !== '' && promotionalGiftName.trim() !== '') {
            if (isCoverExist()) {
                setVali("error");
                    ...
                    <table className="w-full h-full overflow-y-scroll border-s border-black">
                        ...
                        <tbody>
                            {Gifts.map((giftRow, rowIndex) => (
                                <tr key={rowIndex} name={rowIndex} onClick={() => handleRowClick(giftRow.promotionalGiftID)} className="cursor-pointer border-b  border-black hover:bg-gray-400">
                                    ...
                                    <td className="text-center border-r border-black py-2"  >
                                        <button type="button" onClick={() => handleDeleteGift(giftRow.promotionalGiftID)}
